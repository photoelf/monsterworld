...
const SHOP_ITEMS = [
  ...
  { id: 'repel',    cat: 'potions', name: '🚫 Репеллент',         price: 100, desc: 'Отпугивает диких на 100 шагов. Применяется из 🎒 Инвентаря.' },
  { id: 'bigrepel', cat: 'potions', name: '🚫 Крепкий репеллент', price: 300, desc: 'Отпугивает диких на 500 шагов. Применяется из 🎒 Инвентаря.' },
  { id: 'ball_basic',  cat: 'balls', name: '🔮 Сфера ловли',    price: 80,   desc: 'Обычная сфера. Берёт молодняк, на матёрых почти не действует.' },
  { id: 'ball_strong', cat: 'balls', name: '🔷 Крепкая сфера',  price: 240,  desc: 'Ловит в 1.7 раза увереннее обычной.' },
  { id: 'ball_bro',    cat: 'balls', name: '🟣 Братская сфера', price: 700,  desc: 'Ловит в 2.6 раза увереннее обычной — рабочий инструмент на хайлевеле.' },
  { id: 'ball_master', cat: 'balls', name: '🟡 Златая сфера',   price: 7500, desc: 'Ловит кого угодно без осечек. Стоит как небольшой район.' },
...
// Репеллент: сколько шагов без случайных диких встреч даёт каждая склянка
const REPEL_STEPS = { repel: 100, bigrepel: 500 };

// ===== Процедурное описание вида (флейвор для Братопедии) =====
// Собирается из черт вида детерминированно по сиду — без AI, ноль стоимости,
// у одного вида всегда один текст. Тон — тёплый, чуть уличный (под «братву»).

const FLAVOR_TYPE = {
  fire:     ['Греется у костров на пустырях', 'От него вечно тянет дымком', 'В холода лезет к теплотрассе'],
  water:    ['Обожает лужи после дождя', 'Может часами сидеть у фонтана', 'Пахнет свежим дождём'],
  grass:    ['Дремлет в высокой траве', 'Из карманов у него вечно сыплется листва', 'Своих находит по запаху газона'],
  electric: ['Тихо потрескивает, когда его гладишь', 'Заряжает телефон за печеньку', 'Рядом с ним волосы дыбом'],
  ice:      ['Не тает даже в июле', 'Дышит морозным паром', 'Уважает всё холодненькое'],
  psychic:  ['Смотрит так, будто читает мысли', 'Возле него вещи иногда сами шевелятся', 'Задумчивый малый'],
  shadow:   ['Появляется, когда его не ждёшь', 'В темноте видит лучше тебя', 'Молчит и себе на уме'],
  normal:   ['Свой в доску', 'Ничего особенного — но всегда выручит', 'Надёжный, как старый двор'],
};
const FLAVOR_STAT = {
  atk: 'Бьёт первым, вопросы задаёт потом.',
  def: 'С места такого не сдвинешь.',
  spd: 'Носится по району быстрее всех.',
  hp:  'Выносливый — так просто не свалить.',
};
const FLAVOR_STAGE = {
  3: 'Проходит долгий путь: от мелкого пацанёнка до настоящего братана.',
  2: 'С годами заметно матереет.',
  1: 'Каким уродился — таким и остаётся, и его это вполне устраивает.',
};

function speciesFlavor(seed) {
  const sp = getSpecies(seed >>> 0);
  const st = sp.stages[0];
  const rng = mulberry32((seed ^ 0xF1A0) >>> 0);
  const b = st.base;
  const stats = [['hp', b.hp], ['atk', b.atk], ['def', b.def], ['spd', b.spd]];
  const dom = stats.reduce((a, c) => (c[1] > a[1] ? c : a))[0];
  return pick(rng, FLAVOR_TYPE[st.type] || FLAVOR_TYPE.normal) + '. ' +
    FLAVOR_STAT[dom] + ' ' + FLAVOR_STAGE[sp.chainLen];
}
