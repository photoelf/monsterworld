...
function nzGuideRulesSection() {
  return { id: 'nzrules', ic: '☠️', title: 'Правила Nuzlocke', html:
    ...
    '• <b>Макс. лвл</b> — потолок уровня для всей твоей братвы, стартует с 15. Братва растёт только до этого уровня — опыт сверх потолка сгорает без толку, пока он не поднимется (виден в HUD как «макс.лвл»).<br>' +
    'Как растёт потолок: у каждого непобеждённого лидера среди открытых тобой городов есть «топовый братишка» — самый высокоуровневый в его команде. Потолок всегда равен уровню <b>самого слабого</b> из этих топовых братишек — то есть подтягивается к ближайшему по силе бою, который тебе ещё предстоит. Победил лидера — он выпадает из подсчёта, и потолок подтягивается к следующему. Если открытых городов с непобеждёнными лидерами больше не осталось — потолок = топовый братишка последнего побеждённого лидера + 10.<br>' +
    '<i>Пример:</i> ты открыл два города. В городе А лидер не побеждён, его топовый братишка — 20 уровня. В городе Б лидер тоже не побеждён, его топовый братишка — 28 уровня. Потолок = 20 (минимум из двух) — качать братву выше 20 уровня пока бессмысленно. Победил лидера А → в счёте остался только Б → потолок поднимается до 28. Если больше открытых городов с живыми лидерами нет — потолок станет 28 + 10 = 38.<br>' +
